# -*- coding: utf-8 -*-
"""
MS-STGCN (Multi-Scale) 批量性能诊断工具 (v1.3)
=========================================
功能：
1. 加载 MS-STGCN 模型 (包含多尺度 TCN Buffer)。
2. 模拟 Unity 流式输入 (逐帧送入)，验证 Buffer 记忆机制。
3. 统计首次识别帧 (延迟) 和 波峰帧 (稳定性)。
"""
...
PICTURE_SAVE_PATH = "D:/RuanJian/VsCode/Vscode_code/new_hmm/RL/stream/Data1/MS_STGCN_Analysis" # 图片保存路径
...
# 【注意】这里指向 MS-TCN 模型路径
MODEL_PATH = "D:/RuanJian/VsCode/Vscode_code/new_hmm/RL/stream/models_stgcn_v13_mstcn"
...
class StreamingSTGCN_Model(keras.Model):
    def __init__(self, hidden_dim=64, output_dim=32, l2_reg=1e-4, **kwargs):
        ...
        self.tcn1 = StreamingMultiScaleTCN(hidden_dim, l2_reg=l2_reg) # MS-TCN
        self.drop1 = layers.Dropout(0.3)
        self.gcn2 = GraphConv(hidden_dim, self.A, l2_reg)
        self.tcn2 = StreamingMultiScaleTCN(hidden_dim, l2_reg=l2_reg) # MS-TCN
...
def plot_dual_analysis(rec_frames, peak_frames, stats, classes, global_rec_p80, global_peak_avg, save_path):
    ...
    ax1.set_title(f'1. MS-TCN 首次识别帧分布 (First Recognition) - Total: {len(rec_frames)}', fontweight='bold')
    ...
    ax2.set_title(f'2. MS-TCN 波峰置信度帧分布 (Peak Confidence Frame)', fontweight='bold')
    ...
    print(f"📈 MS-STGCN 分析图表已保存至: {save_path}")
...
if __name__ == "__main__":
    ...
    print(f"🚀 开始 MS-STGCN 全序列流式扫描 (共 {len(test_files)} 个样本)...")
    print("⚠️  注意：由于 MS-TCN 感受野为 17 帧，前 17 帧的预测可能不稳定。")
    ...
    for item in tqdm(test_files, desc="Processing"):
        ...
        for i in range(len(data)):
            ...
            # (Batch=1, Time=1, Feat) -> 触发 Stream Logic
    ...
    if all_rec_frames:
        ...
        save_path = os.path.join(PICTURE_SAVE_PATH, "MS_STGCN_Performance.png")
