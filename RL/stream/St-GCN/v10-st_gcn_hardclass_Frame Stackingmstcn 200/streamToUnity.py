# -*- coding: utf-8 -*-
"""
ST-GCN v13.6 模型转换脚本 (Keras -> ONNX for Unity Sentis)
=====================================================
修复日志:
1. [关键修复] 补全 StreamingMultiScaleTCN 的 build 方法，解决权重数量不匹配 (12 vs 17) 问题。
2. [路径修正] 分离 STGCN_PATH (Phase 1) 和 RL_PATH (Phase 2)，解决 PPO Actor 找不到文件的问题。
3. [维度修复] 保持对 Call 方法中 Squeeze 的修复。
"""
...
# --- 1. 路径配置 (请仔细核对) ---
BASE_PATH = "D:/RuanJian/VsCode/Vscode_code/new_hmm/RL/stream/St-GCN"

# Phase 1 结果路径 (ST-GCN 特征提取器 & 分类头)
STGCN_PATH = os.path.join(BASE_PATH, "models_stgcn_v13_6_mstcn_193fps")

# Phase 2 结果路径 (PPO Actor)
RL_PATH = os.path.join(BASE_PATH, "models_rl_v13_6_mstcn_193fps")

# ONNX 保存目标位置 (默认保存在 RL 文件夹下，方便 Unity 导入)
SAVE_PATH = RL_PATH 
...
# 输出文件
ONNX_FE_PATH = os.path.join(SAVE_PATH, "feature_extractor_v13_6.onnx")
ONNX_CH_PATH = os.path.join(SAVE_PATH, "classifier_head_v13_6.onnx")
ONNX_AGENT_PATH = os.path.join(SAVE_PATH, "ppo_actor_v13_6.onnx")

# 全局参数
L2_REG_VALUE = 1e-4

# ============================================================================
# A. 原始训练模型定义 (用于加载权重 - 必须与训练脚本完全一致)
# ============================================================================
...
class StreamingMultiScaleTCN(layers.Layer):
    ...
    # 【关键修复】必须包含 build 方法以定义 buffer 权重，否则 Keras 加载时会报错
    ...
    def call(self, inputs, training=None): 
        ...
        return self.conv_out(tf.concat([x1, x2, x3, x4, x5], axis=-1))
...
# ============================================================================
# B. 导出专用模型结构 (Barracuda/Sentis 兼容版 - MS-TCN + MatMul Pool)
# ============================================================================
...
class StreamingMSTCN_Export(layers.Layer):
    ...
    def call(self, inputs, buffers):
        ...
        final_out = self.conv_out(merged)
...
def convert_fe_v13():
    print("\n[1/3] 转换 ST-GCN v13.6 (5-Branch MS-TCN)...")
    ...
    try:
        train_model.load_weights(FE_WEIGHTS_PATH)
        print("  - 训练权重加载成功。")
    except Exception as e:
        print(f"  x 权重加载失败: {e}"); return

    export_model = STGCN_Export_Model_v13(output_dim=32)
    
    # B1:2, B2:4, B3:16, B4:64, B5:96
    buf_sizes = [2, 4, 16, 64, 96]
    dummy_bufs = [tf.zeros((1, s, 64)) for s in buf_sizes]
    
    input_list = [tf.zeros((1, 1, 218))] + dummy_bufs + dummy_bufs
    export_model(input_list)
    ...
    print(f"✅ [成功] ST-GCN v13.6 已保存: {ONNX_FE_PATH}")
...
def convert_actor():
    ...
    actor_model = keras.Model(inp, out)
    
    try:
        actor_model.load_weights(RL_AGENT_PATH)
        print("  - Actor 权重加载成功。")
    except Exception as e:
        print(f"  x Actor 权重失败: {e}"); return
    ...
    print(f"✅ [成功] PPO Actor 已保存: {ONNX_AGENT_PATH}")
