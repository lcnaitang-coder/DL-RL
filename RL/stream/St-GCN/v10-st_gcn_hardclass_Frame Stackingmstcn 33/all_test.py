# -*- coding: utf-8 -*-
"""
MS-STGCN (Multi-Scale v13.4) 批量性能诊断工具
=========================================
功能：
1. 加载 v13.4 大视野版 MS-STGCN 模型 (33帧视野)。
2. 模拟 Unity 流式输入，验证扩大视野后的延迟和准确率。
"""
...
PICTURE_SAVE_PATH = "D:/RuanJian/VsCode/Vscode_code/new_hmm/RL/stream/Data1/MS_STGCN_Analysis"
...
# 指向 v13.4 训练出的模型路径
MODEL_PATH = "D:/RuanJian/VsCode/Vscode_code/new_hmm/RL/stream/models_stgcn_v13_mstcn_33fps"
...
# ============================================================================
# 2. MS-STGCN 模型定义 (已同步 v13.4 训练脚本)
# ============================================================================
...
class StreamingMultiScaleTCN(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.buf_mid   = self.add_weight(name='buf_m', shape=(1, 8, input_shape[-1]), initializer='zeros', trainable=False) # buf=8
        self.buf_large = self.add_weight(name='buf_l', shape=(1, 32, input_shape[-1]), initializer='zeros', trainable=False) # buf=32
...
class StreamingSTGCN_Model(keras.Model):
    def __init__(self, hidden_dim=64, output_dim=32, l2_reg=1e-4, **kwargs):
        ...
        self.tcn1 = StreamingMultiScaleTCN(hidden_dim, l2_reg=l2_reg) # MS-TCN v13.4
        self.drop1 = layers.Dropout(0.3)
        self.gcn2 = GraphConv(hidden_dim, self.A, l2_reg)
        self.tcn2 = StreamingMultiScaleTCN(hidden_dim, l2_reg=l2_reg) # MS-TCN v13.4
    ...
    def call(self, inputs, training=False):
        ...
        return tf.squeeze(out, axis=1) # 仅用于流式
...
def plot_dual_analysis(rec_frames, peak_frames, stats, classes, global_rec_p80, global_peak_avg, save_path):
    ...
    ax1.set_title(f'1. MS-TCN (33 Frames) 首次识别帧分布 - Total: {len(rec_frames)}', fontweight='bold')
    ...
    print(f"📈 MS-STGCN (v13.4) 分析图表已保存至: {save_path}")
...
if __name__ == "__main__":
    ...
    # 2. 加载模型
    print("Building MS-STGCN Model (33 Frames Receptive Field)...")
    ...
    print(f"✅ Classifier Head Loaded from {CH_PATH}")
    
    print(f"🚀 开始 MS-STGCN (v13.4) 全序列流式扫描 (共 {len(test_files)} 个样本)...")
    print("⚠️  注意：由于视野扩大到 33 帧，前 33 帧 (约0.55秒) 预测可能不稳定。")
    ...
    if all_rec_frames:
        ...
        save_path = os.path.join(PICTURE_SAVE_PATH, "MS_STGCN_LargeField_Performance.png")
